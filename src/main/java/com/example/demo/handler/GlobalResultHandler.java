...
public class GlobalResultHandler implements ResponseBodyAdvice<Object> {
    ...
    public boolean supports(MethodParameter returnType, Class<? extends HttpMessageConverter<?>> converterType) {
        return returnType.getDeclaringClass().getPackage().getName().contains(PACKAGE_PATH)
                && !returnType.getMethod().getReturnType().isAssignableFrom(Void.TYPE)
                && !returnType.getMethod().getReturnType().isAssignableFrom(String.class)
                && !returnType.getMethod().getReturnType().isAssignableFrom(ResultResponse.class);
    }
